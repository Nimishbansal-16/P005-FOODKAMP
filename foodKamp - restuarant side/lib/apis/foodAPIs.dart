...
updateCategory(String category,AuthNotifier authNotifier) async {


  CollectionReference restRef = Firestore.instance.collection('restaurants');
...
updateOpentime(String opentime,AuthNotifier authNotifier) async {
...
updateClosetime(String closetime,AuthNotifier authNotifier) async {
...
addNewItem(String itemName, int price, int totalQty,int time, BuildContext context, String restaurantID2) async {
  pr = new ProgressDialog(context, type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  try {
    CollectionReference itemRef = Firestore.instance.collection('restaurants').document(restaurantID2).collection('items');
    await itemRef
      .document().setData({"item_name": itemName, "price": price, "total_qty": totalQty, "time": time})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
...
editItem(String itemName, int price, int totalQty,int time, BuildContext context, String id, String restaurantID2) async {
  pr = new ProgressDialog(context, type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  try {
    CollectionReference itemRef = Firestore.instance.collection('restaurants').document(restaurantID2).collection('items');
    await itemRef
      .document(id).setData({"item_name": itemName, "price": price, "total_qty": totalQty, "time": time})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
...
deleteItem(String id, BuildContext context, String restaurantID2) async {
  pr = new ProgressDialog(context, type: ProgressDialogType.Normal, isDismissible: false, showLogs: false);
  pr.show();
  try {
    CollectionReference itemRef = Firestore.instance.collection('restaurants').document(restaurantID2).collection('items');
...
orderReceived(String id, BuildContext context) async {
  ...
  try {
    ...
    await ordersRef
      .document(id).updateData({'is_delivered': 'Delivered'})
      .catchError((e) => print(e))
      .then((value) => print("Success"));
  } catch (error) {
    pr.hide().then((isHidden) {
